...
export class LiveNewsPanel extends Panel {
  ...
  private get embedOrigin(): string {
    ...
    try { return new URL(getRemoteApiBaseUrl()).origin; } catch { return window.location.origin || 'https://theveritas.netlify.app'; }
  }
  ...
  private static resolveYouTubeOrigin(): string | null {
    const fallbackOrigin = window.location.origin || 'https://theveritas.netlify.app';
  ...
  private async renderDesktopEmbedAsync(force = false): Promise<void> {
    ...
    params.set('origin', this.youtubeOrigin || window.location.origin || 'https://theveritas.netlify.app');
