...
function generateFallbackData(metal, range) {
  const isGold = metal === "gold";
  const baseValue = isGold ? 2650.40 : 31.25;
  const change = isGold ? 18.60 : 0.42;
  const changePercent = isGold ? 0.71 : 1.36;
  const currency = "USD";
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    const noise =
      (Math.sin(i * 0.5) * 0.4 + Math.cos(i * 0.8) * 0.2) *
      (baseValue * 0.004);
...
export default async function handler(req, res) {
  const { metal = "gold", range = "1d" } = req.query;
  const lowerMetal = metal.toLowerCase();

  if (lowerMetal !== "gold" && lowerMetal !== "silver") {
    return res.status(400).json({ error: "Invalid metal. Use 'gold' or 'silver'." });
  }

  const cacheKey = `${lowerMetal}_${range}`;
  const now = Date.now();
  ...
  const yahooSymbol = lowerMetal === "gold" ? "GC=F" : "SI=F";
  ...
  try {
    ...
    const previousClose =
      typeof meta.previousClose === "number"
        ? meta.previousClose
        : meta.chartPreviousClose || points[0].value;
    ...
    console.error(`Error loading commodities chart for ${lowerMetal}:`, error);
    const fallback = generateFallbackData(lowerMetal, range);
